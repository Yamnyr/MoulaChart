...
with st.sidebar:
    ...
    st.markdown("---")
    account_filter = st.multiselect("Compte", ["DEFAULT", "PEA"], default=["DEFAULT", "PEA"])
...
kpi(c1, "💰 Total investi", f"{total_invested:,.0f} €")
kpi(c2, "📥 Dépôts totaux", f"{total_deposited:,.0f} €")
kpi(c3, "🎁 Saveback reçu", f"{total_saveback:.2f} €", "Cashback TR 1%")
kpi(c4, "🏦 Intérêts reçus", f"{total_interest:.2f} €")
kpi(c5, "💳 Dépenses carte", f"{total_spent:,.0f} €", f"{len(cards)} transactions", False)

# ════════════════════════════════════════════════════════════════
# SECTION 1 — INVESTISSEMENTS
# ════════════════════════════════════════════════════════════════
st.markdown('<div class="section-title">📊 Mes Investissements</div>', unsafe_allow_html=True)
...
with tab_inv1:
        ...
        if cum_data:
            ...
            for etf in df_cum["etf"].unique():
                ...
                fig.add_trace(go.Scatter(
                    ...
                    hovertemplate=f"<b>{etf}</b><br>%{{x|%d/%m/%Y}}<br>%{{y:,.0f}} €<extra></extra>"
                ))
            fig.update_layout(
                ...
                yaxis=dict(showgrid=True, gridcolor="rgba(102,126,234,0.1)", title="€ investi"),
    ...
    with col_r:
        ...
        fig2.update_layout(
            ...
            yaxis=dict(showgrid=True, gridcolor="rgba(102,126,234,0.1)", title="€"),
...
with tab_inv2:
    ...
    with col_b:
        # Shares accumulated per ETF
        shares_by_etf = buys.groupby("etf_name")["shares"].sum().reset_index()
        shares_by_etf.columns = ["ETF", "Parts"]
        colors_bar = [ETF_COLORS.get(e, "#64748b") for e in shares_by_etf["ETF"]]
        fig_shares = go.Figure(go.Bar(
            x=shares_by_etf["ETF"], y=shares_by_etf["Parts"],
            marker_color=colors_bar,
            hovertemplate="<b>%{x}</b><br>%{y:.4f} parts<extra></extra>",
            text=shares_by_etf["Parts"].apply(lambda v: f"{v:.4f}"),
            textposition="outside"
        ))
        fig_shares.update_layout(
            title="Parts accumulées par ETF", template="plotly_dark",
            plot_bgcolor="rgba(15,23,42,0.5)", paper_bgcolor="rgba(0,0,0,0)",
            font=dict(color="#e2e8f0"), height=380,
            yaxis=dict(showgrid=True, gridcolor="rgba(102,126,234,0.1)", title="Nombre de parts"),
            margin=dict(l=0, r=0, t=50, b=0)
        )
...
with tab_inv3:
    ...
    etf_detail.columns = ["ETF", "Compte", "Nb achats", "Investi (€)", "Parts totales", "Prix moyen (€)"]
    st.dataframe(
        etf_detail.style.format({
            "Investi (€)": "{:,.2f}", "Parts totales": "{:.6f}", "Prix moyen (€)": "{:.2f}"
    ...
    with c1:
        ...
        fig_sav.update_layout(title="Saveback mensuel (€)", template="plotly_dark",
            ...
            xaxis=dict(tickangle=45), margin=dict(l=0,r=0,t=40,b=60))
...
with tab_dep1:
    st.markdown("##### Flux de trésorerie — Entrées → Catégories → Marchands")
    ...
    merchants = top_merchants_per_cat[["cat", "merchant"]].drop_duplicates()
...
with tab_dep3:
    ...
    fig_top.update_layout(
        ...
        template="plotly_dark", plot_bgcolor="rgba(15,23,42,0.5)",
        paper_bgcolor="rgba(0,0,0,0)", font=dict(color="#e2e8f0"),
        height=580, xaxis=dict(title="€ total", showgrid=True, gridcolor="rgba(102,126,234,0.1)"),
    ...
    st.dataframe(show.sort_values("Date", ascending=False).style.format({"Montant (€)": "{:.2f}"}),
                 use_container_width=True, hide_index=True)
